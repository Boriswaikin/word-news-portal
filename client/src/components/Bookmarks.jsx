...
export default function Bookmarks() {
  ...
  async function updateBookmarks(updateID) {
    const data = await fetch(
      `${process.env.REACT_APP_API_URL}/todos/` + updateID,
      {
        method: "PATCH",
        headers: {
          "Content-Type": "application/json",
    ...
    if (data.ok) {
      ...
      setBookmarks((prev) =>
        prev.map(({ id, displayTitle, ...prev }) => ({
          ...prev,
          displayTitle: id === updateID ? newTitle : displayTitle,
        }))
      );
    }
  }
            ...
            bookmarks.map((item, index) => {
              return (
                <li key={index} className="bookmark-item">
                  <Link to={`/news/${index}`} className="bookmark-link">
                    {item.displayTitle}
                  </Link>
                  <p>{item.category}</p>
                  <p className="bookmarks-publishDate">{item.publishDate}</p>
                  <button
                    className="bookmarks edit"
                    onClick={() => {
                      setNewID(item.id);
                      setEdit(true);
                    }}
